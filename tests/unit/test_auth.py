...
async def test_register_success(async_client, db_session):
    ...
    response = await async_client.post(
        "/api/auth/register", json={"email": test_email, "password": "password123"}
    )
...
async def test_register_duplicate_email(async_client):
    ...
    response = await async_client.post(
        "/api/auth/register", json={"email": test_email, "password": "different123"}
    )
...
async def test_login_success(async_client, db_session):
    ...
    response = await async_client.post(
        "/api/auth/token", data={"username": test_email, "password": test_password}
    )
...
async def test_login_invalid_credentials(async_client):
    ...
    response = await async_client.post(
        "/api/auth/token",
        data={"username": f"wrong_{uuid.uuid4()}@example.com", "password": "wrongpass"},
    )
...
async def test_register_invalid_email(async_client):
    ...
    response = await async_client.post(
        "/api/auth/register", json={"email": "invalid_email", "password": "password123"}
    )
...
async def test_register_short_password(async_client):
    ...
    response = await async_client.post(
        "/api/auth/register",
        json={"email": f"test_{uuid.uuid4()}@example.com", "password": "short"},
    )
