...
@pytest.mark.asyncio
async def test_get_recommendation_success(async_client, auth_token, db_session, mock_redis):
...
async def test_get_recommendation_not_found(async_client, auth_token, mock_redis):
    ...
    headers = {"Authorization": f"Bearer {auth_token}"}
    response = await async_client.get(
        f"/api/recommendations/nonexistent-id",
        headers=headers
    )
...
@pytest.mark.asyncio
async def test_get_recommendation_cache_miss(async_client, auth_token, db_session, mock_redis):
